...
if not len(lib['books']):
    ...
    lib.add_books(read_list_books('mylib\\books.txt'))       # У меня на Windows сработал путь через "\\", вместо "/"
...
@app.route('/search', methods=['GET', 'POST'])      # Добавил поиск по книгам на сайте
def api_search():
    searchd = request.form.get('searchdata')
    if request.method == 'POST':
        return render_template('search.html', books=lib.search(searchdata=searchd))
    return render_template('search.html', books=lib.search(searchdata=searchd))
